...
### `/checkpoint`
Saves current working context to `.j2/current.md` with four sections: `## In Progress`, `## Just Completed`, `## Next Steps`, `## Open Questions`. Can be run at any point — it is not a quality gate. After writing `current.md`, stages all changes with `git add -A`, inspects `git diff --cached --stat` to write a meaningful one-line commit message, commits, and pushes to the remote. If no upstream is set, it sets one automatically.
